...
const Components = Object.fromEntries(
	Object.values(Fields).map(({ name, save }) => [name, save])
);
...
function BlockSave(blueprint) {
	return ({ attributes, innerBlocks, ...props }) => {
		...
		const _blockSave = normalizeComponentList(
			blueprint?.blockSave,
			_blockEdit[0]
		);
		...
		const blockClassNames = classNames(
			[
				...
				...((Array.isArray(blockSave.className) &&
					blockSave.className) || [blockSave.className]),
				...((Array.isArray(blockSave.viewClassName) &&
					blockSave.viewClassName) || [blockSave.viewClassName]),
			],
			blockContext
		);
