...
function evaluateCondition(string = "", context = {}) {
	...
	let before = replaceTokens(
		string.slice(0, string.indexOf(operand)).trim(),
		context
	);
	let after = replaceTokens(
		string.slice(string.indexOf(operand) + operand.length).trim(),
		context
	);

	if (
		(before.slice(0, 1) === "'" && before.slice(-1) === "'") ||
		(before.slice(0, 1) === '"' && before.slice(-1) === '"')
	) {
