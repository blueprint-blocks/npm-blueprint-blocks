...
function render(
	...
	{ clientId, blockName, attributes, setAttributes = null, context = {} },
	Components = {}
) {
	...
	const jsxAttributes = Object.fromEntries(
		Object.entries(props).map(([name, value]) => {
			if (
				typeof value === "string" &&
				EXCLUDED_ATTRIBUTES.includes(name) === false
			) {
				return [
					name,
					replaceTokens(value, {
	...
	if ("display" in jsxAttributes) {
		jsxAttributes.display = evaluateConditionalString(
			props.display,
			context
		);
	}
	...
	const jsxClassNames = classNames(
		[
			...
			...((context?.context === "edit" &&
				((Array.isArray(editorClassName) && editorClassName) || [
					editorClassName,
				])) ||
				[]),
			...((context?.context === "save" &&
				((Array.isArray(viewClassName) && viewClassName) || [
					viewClassName,
				])) ||
				[]),
		],
	...
	const jsxTagName = replaceTokens(tagName, {
		...context,
		attribute: { value: attributeValue },
	});

	let Component = jsxTagName;
	...
	return (
		<Component {...jsxAttributes} blockName={blockName}>
			{renderJsxArray(
				{
					clientId,
					blockName,
 ...
 * @param {array} jsx
 */
function renderJsxArray({ jsx = [], ...blockProps }, Components = {}) {
